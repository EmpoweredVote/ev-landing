...
(function () {
  ...
  onReady(function () {
    ...
    var BEAM_LOADS = ['circle', 'line', 'button'];   // the ball, the yellow line, and the Fallacy Finders button (no notched triangle); beamPick rotates and avoids repeats

    // ── Fallacy Finders lockup, as canvas path data ────────────────────────────────────
    // GENERATED by tools/gen-fallacy-card-art.mjs from icons/fallacy-finders-logo-lg-*.svg.
    // Do not hand-edit; re-run the tool if the brand artwork changes.
    //
    // Path data rather than an <img> of the SVG on purpose: an SVG loaded by `src` and drawn to a
    // canvas TAINTS it under file://, and every pixel test in tests/ runs from file:// — a tainted
    // beam canvas makes tests/layout/beam-clears-metarow.cjs throw SecurityError forever. Path data
    // has no origin, so it cannot taint. The two brand variants share one geometry and differ only
    // in the wordmark teal, so the colours are ROLES resolved per theme.
    var FF_ART = { x0: 0, y0: -7, w: 530, h: 183,
      light: {"coral":"#FF5740","ink":"#00657C","spark":"#FFD426"},
      dark: {"coral":"#FF5740","ink":"#59B0C4","spark":"#FFD426"},
      evenodd: [13,14],
      roles: ["coral","coral","coral","coral","coral","coral","ink","ink","ink","ink","ink","ink","ink","ink","ink","spark","spark","spark","spark","spark","spark"],
      d: [
        'M33.4 103H45.4V142.1H33.4V103ZM32.5 90.7C32.5 88.8 33.1 87.2 34.4 85.8C35.8 84.4 37.5 83.7 39.4 83.7C41.3 83.7 42.9 84.4 44.3 85.8C45.6 87.2 46.3 88.8 46.3 90.7C46.3 92.7 45.6 94.3 44.3 95.7C42.9 97.1 41.3 97.7 39.4 97.7C37.5 97.7 35.8 97.1 34.4 95.7C33.1 94.3 32.5 92.7 32.5 90.7ZM7.9 112.7H0V103H7.9V96.1C7.9 94 8.1 92 8.5 90.1C8.8 88.2 9.5 86.5 10.6 85C11.7 83.6 13.2 82.5 15.2 81.7C17.2 80.8 19.8 80.4 23 80.4C24.3 80.4 25.5 80.4 26.6 80.5C27.8 80.6 28.9 80.8 30.1 81.1L29.5 91.3C28.8 91 28.2 90.8 27.6 90.7C27 90.6 26.3 90.5 25.6 90.5C23.7 90.5 22.2 90.9 21.3 91.8C20.3 92.6 19.9 94.3 19.9 96.8V103H28.7V112.7H19.9V142.1H7.9V112.7Z',
        'M54 103H65.5V108.3H65.6C66 107.6 66.5 106.8 67.2 106.1C67.9 105.3 68.7 104.7 69.7 104.1C70.6 103.5 71.7 103 73 102.6C74.2 102.3 75.5 102.1 76.9 102.1C80 102.1 82.4 102.5 84.3 103.5C86.1 104.4 87.6 105.7 88.6 107.4C89.6 109 90.4 111 90.7 113.2C91.1 115.5 91.3 117.9 91.3 120.5V142.1H79.3V123C79.3 121.8 79.3 120.7 79.2 119.5C79.1 118.3 78.9 117.1 78.5 116.1C78.1 115.1 77.5 114.3 76.6 113.6C75.8 113 74.7 112.7 73.1 112.7C71.6 112.7 70.3 113 69.4 113.6C68.4 114.1 67.7 114.8 67.1 115.8C66.7 116.7 66.3 117.8 66.2 118.9C66 120.1 65.9 121.3 65.9 122.6V142.1H54V103Z',
        'M243.2 68.1C242.4 70.2 241.6 72 240.9 73.6C240.1 75.2 239.2 76.6 238 77.6C236.9 78.8 235.5 79.6 233.7 80.1C231.9 80.7 229.6 81 226.8 81C223.7 81 220.7 80.5 217.8 79.6L219.4 69.7C221.2 70.5 223.1 70.9 225 70.9C226.3 70.9 227.4 70.7 228.3 70.4C229.1 70.1 229.8 69.7 230.4 69.1C230.9 68.6 231.4 67.9 231.8 67.1C232.2 66.3 232.6 65.4 233 64.3L233.9 62L216.9 22.7H229.8L239.8 48.5H239.9L248.5 22.7H260.7L243.2 68.1Z',
        'M402.8 8.3C413.9 -2.8 431.9 -2.8 443 8.3L443.9 9.2C455 20.3 455 38.3 443.9 49.5L419.8 73.5C410.3 83.1 395.6 84.4 384.6 77.4L377.6 84.3L378.7 74.6L368 75.9L371.9 79.7L367.8 75.7L375.1 68.2C367.8 57.1 369 42.1 378.7 32.4L402.8 8.3ZM433.8 17.5C427.7 11.5 418 11.5 411.9 17.5L387.9 41.5C383.3 46.1 382.2 52.8 384.5 58.4L396 46.5C398.7 43.7 403.3 43.6 406.1 46.4L406.4 46.7C408.9 49.4 408.8 53.7 406.1 56.3L394.3 67.9C399.8 69.9 406.2 68.7 410.7 64.3L434.7 40.3C440.7 34.2 440.7 24.5 434.7 18.4L433.8 17.5Z',
        'M357.9 93.7L368.7 92.5L364.8 88.7L368.9 92.6L361.6 100.2C368.9 111.2 367.7 126.3 358 136L333.9 160C322.8 171.1 304.8 171.1 293.7 160L292.8 159.1C281.7 148 281.7 130 292.8 118.9L316.8 94.9C326.4 85.3 341.1 84 352.1 90.9L359.1 84.1L357.9 93.7ZM342.4 100.5C336.9 98.4 330.4 99.6 326 104L302 128.1C295.9 134.1 295.9 143.9 302 149.9L302.9 150.8C308.9 156.9 318.7 156.9 324.7 150.8L348.8 126.8C353.3 122.2 354.5 115.6 352.1 110L340.7 121.8C337.9 124.7 333.4 124.7 330.6 121.9L330.3 121.7C327.8 118.9 327.9 114.7 330.6 112L342.4 100.5Z',
        'M379.1 105.5C380.9 105.1 382.8 106.2 383.2 108L385.3 116.3C385.7 118.1 384.6 119.9 382.8 120.4C381 120.8 379.1 119.7 378.7 117.9L376.6 109.7C376.2 107.8 377.3 106 379.1 105.5Z',
        'M387.8 97C388.1 95.2 389.9 94 391.8 94.3L400.1 96C401.9 96.4 403.1 98.2 402.7 100C402.4 101.9 400.6 103.1 398.7 102.7L390.4 101C388.6 100.6 387.4 98.8 387.8 97Z',
        'M379.5 89.8L373.7 95.7L373 89.1L379.5 89.8Z',
        'M363.6 79.7L357.1 79L362.9 73.2L363.6 79.7Z',
        'M333.8 68.9C334.2 67 336 65.8 337.9 66.2L346.2 67.9C348 68.2 349.2 70 348.8 71.9C348.5 73.7 346.7 74.9 344.8 74.5L336.5 72.9C334.7 72.5 333.5 70.7 333.8 68.9Z',
        'M353.8 48.5C355.6 48 357.4 49.1 357.9 51L360 59.2C360.4 61 359.3 62.9 357.5 63.3C355.7 63.8 353.8 62.7 353.4 60.9L351.3 52.6C350.9 50.8 352 49 353.8 48.5Z'
      ] };
    var FF_PATHS = null;   // lazily built Path2D objects, one per entry in FF_ART.d
    ...
    // The button as the crew carries it: UPSIDE DOWN. The whole card is rotated 180 degrees, which is
    // the gag — the sign for the next feature is being carried in and nobody has established which
    // way up it goes.
    //
    // Every internal proportion is the real button's, scaled by `k` off the measured height: its 24px
    // padding, its 64px logo box, its 16px corner radius. So the thing between them reads as the same
    // object as the five buttons in the list above, just enormous next to an 84px figure.
    //
    // The logo HUGS ONE END rather than sitting centred, faithful to the buttons, which right-align
    // their content so the five icons line up in the column. Because the card is rotated, that end
    // reads opposite in world space, and it swaps sides as the crew reverses direction — accepted.
    function drawButtonCard(ctx, cx, cy, cw, ch) {
      var k = ch / 102;                       // the button measures 102px tall at >=1025px
      var pad = 24 * k, logoH = 64 * k;
      ...
      ctx.strokeStyle = cssVar('--border', '#E5E7EB'); ctx.lineWidth = 1; ctx.stroke();
      // the lockup, inset by the button's own padding, scaled to the button's own logo box
      var lw = logoH * (FF_ART.w / FF_ART.h);
      var maxW = cw - pad * 2;
      var ls = (lw > maxW) ? (maxW / lw) : 1;
      ctx.translate(cw / 2 - pad - lw * ls, -logoH * ls / 2);
      ctx.scale((logoH * ls) / FF_ART.h, (logoH * ls) / FF_ART.h);
    ...
    // How far off-screen a pass has to start and finish. The crew's own bodies only need ~110px, but
    // a load carried BETWEEN them sticks out half its width past the middle, so an oversized one has
    // to be given room or its far end appears out of nothing at the edge. Shared by beamPick (which
    // sets the spawn x) and the draw branch (which decides when to turn around) so the two cannot
    // disagree — they used to both hardcode 110.
    function loadMargin(e, load, w) {
      if (load !== 'button') return 110;
      return buttonCardSize(e, w).w / 2 + 110;
    }
    ...
    function beamPick(e, w) {
      ...
      if (goCarry) {
        ...
        var m = loadMargin(e, a, w);   // NOT a bare 110: an oversized load needs room for its far end
        e.scene = 'carry'; e.load = a; e.dwell = 1.1; e.bx = e.dir > 0 ? -m : w + m;
      } else {
    ...
    function tick() {
      ...
      entries.forEach(function (e) {
        ...
        if (spec.mode === 'beam') {
          ...
          // then walks back carrying the next load in the rotation (ball → line → button).
          // Start load is randomized so you don't wait a full ~28s off-screen traverse to see it swap.
          var isBtn = (e.load === 'button');
          var card = isBtn ? buttonCardSize(e, w) : null;
          // The button is the whole showcase button at full size — 340x102 on a wide screen — so it
          // changes the shape of the pass, not just what is drawn between them:
          //   gap    the crew stands at its ENDS rather than shoulder to shoulder
          //   margin the whole card has to clear the edge before they turn, or the far end pops out
          //          of nothing (this is also why beamPick asks loadMargin for the spawn x)
          //   speed  FASTER, not slower. The card covers part of .meta-row on the way past and pace
          //          is the only lever on how long that lasts (~19s instead of ~32s). The strain
          ...
          var endMargin = loadMargin(e, e.load, w);
          ...
          if (e.propGone) {
            ...
            // How high they hold it. The rig's hands sit 32px above the feet, and putting the card's
            // bottom edge THERE is what makes it read as carried rather than walked beside — compared
            // five variants at 1:1 and this was the difference.
            //
            // But hold height is also meta-row coverage, and the cost is lopsided, so the two cases
            // are split. Crew headroom is 93px at every width; the card is ~102px tall:
            //   >=901px  the row is ONE 25px line and 20px of hold already covers it completely
            //            (card top 122 above feet vs 93 of headroom). The extra 12px reaches into
          ...
          if (e._pickup > 0) {
            ...
            // strained: folded deeper, shorter steps, arms hanging, a sag onto each planted foot
